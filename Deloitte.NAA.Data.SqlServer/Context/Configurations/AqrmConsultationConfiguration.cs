...
{
    public class AqrmConsultationConfiguration : IEntityTypeConfiguration<AqrmConsultation>
    {
        public void Configure(EntityTypeBuilder<AqrmConsultation> builder)
        {
            builder.ToTable("AQRMConsultation");

            //builder.HasIndex(e => e.ReviewBaseId)
            //    .HasName("IDX_AQRMConsultation:Composite5422");

            //builder.HasIndex(e => new { e.ReviewBaseId, e.CurrentStateId })
            //    .HasName("IDX_AQRMConsultation:Composite23641");

            //builder.HasIndex(e => new { e.CurrentStateId, e.PriorityId, e.IsRelatedtoMorethanOneEntity, e.ReviewBaseId })
            //    .HasName("IDX_AQRMConsultation:Composite21765");

            //builder.HasIndex(e => new { e.ReviewBaseId, e.QueryDescription, e.SubjectMatterId, e.StateChangedDate, e.PriorityId, e.Notes, e.CurrentStateId })
            //    .HasName("IDX_AQRMConsultation:Composite124231");

            builder.Property(e => e.AqrmConsultationId)
                .HasColumnName("AQRMConsultationID")
                .HasMaxLength(50)
                .HasDefaultValueSql("(N'AQRM_'+CONVERT([nvarchar](50),NEXT VALUE FOR [dbo].[AQRMConsultationIDSeq],(0)))");
            ...
            builder.Property(e => e.ReferenceGuidance).HasMaxLength(500);
            ...
            builder.HasOne(d => d.CurrentState)
                .WithMany(p => p.Aqrmconsultation)
                .HasForeignKey(d => d.CurrentStateId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_AQRMConsultation_State");

            builder.HasOne(d => d.EpsignoffByNavigation)
                .WithMany(p => p.AqrmconsultationEpsignoffByNavigation)
                .HasForeignKey(d => d.EpsignoffBy)
                .HasConstraintName("FK_AQRMConsultation_User1");

            builder.HasOne(d => d.EqcrsignoffByNavigation)
                .WithMany(p => p.AqrmconsultationEqcrsignoffByNavigation)
                .HasForeignKey(d => d.EqcrsignoffBy)
                .HasConstraintName("FK_AQRMConsultation_User3");

            builder.HasOne(d => d.NaapartnerSignoffByNavigation)
                .WithMany(p => p.AqrmconsultationNaapartnerSignoffByNavigation)
                .HasForeignKey(d => d.NaapartnerSignoffBy)
                .HasConstraintName("FK_AQRMConsultation_User2");

            builder.HasOne(d => d.ReviewBase)
                .WithMany(p => p.Aqrmconsultation)
                .HasForeignKey(d => d.ReviewBaseId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_AQRMConsultation_ReviewBase");

            builder.HasOne(d => d.StateChangedByUser)
                .WithMany(p => p.AqrmconsultationStateChangedByUser)
                .HasForeignKey(d => d.StateChangedByUserId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_AQRMConsultation_User");
        }
    }
}
